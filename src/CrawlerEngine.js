...
export class CrawlerEngine {
  ...
  async start() {
    ...
    // await this.downloader.ensureTargetDirectoryExists();
    ...
    console.log(
      `   Unique URLs visited: ${this.visitedTracker.getVisitedCount()}`
    );
  }
  ...
  async crawl(url, depth) {
    ...
    try {
      console.log(
        `${"  ".repeat(depth)}📄 Crawling (depth ${depth}): ${normalizedUrl}`
      );
      ...
      console.log(
        `${"  ".repeat(depth)}⚠️  Failed to crawl ${normalizedUrl}: ${
          error.message
        }`
      );
